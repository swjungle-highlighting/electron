...
def audioProcess(url_id):
    ...
    print("audio folder+'\\'+target", folder+'\\'+target)
    out, err = (
        ffmpeg
            .input(folder+'\\'+target)
...
def videoProcess(url_id):
    ...
    print("video folder+'\\'+target", folder+'\\'+target)
...
def _do_subprocess(input_file, duration, index, audio, video) :
    ...
    # test = os.popen(" ".join(ffmpeg_call)).read()
    # os.system(" ".join(ffmpeg_call))
    test = subprocess.check_output(" ".join(ffmpeg_call), stderr=subprocess.STDOUT)
    print('do test',test)
...
if __name__ == "__main__":
    ...
    result=streamProcess('https://www.youtube.com/watch?v=gdZLi9oWNZg')
